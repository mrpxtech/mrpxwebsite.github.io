...
def SplitLinesRecursive(theta, rho, startIdx, endIdx, params):
    ...
    # idx:Each row corresponds to each segment, 
    # where the first column is the start and the second column is the end 
    ...
    if splitIdx != -1:
        ...
        #stacking and making sure it comes as an np.array
        alpha=np.row_stack((alpha1,alpha2))
        r=np.row_stack((r1,r2))
        idx=np.row_stack((idx1,idx2))

    # print("start and end indexes:",startIdx,endIdx)
    return alpha, r, idx
...
def FindSplit(theta, rho, alpha, r, params):
    ...
    if d[splitIdx]<LINE_POINT_DIST_THRESHOLD:
        return -1
    else:
        if splitIdx<MIN_POINTS_PER_SEGMENT or splitIdx>len(theta)-MIN_POINTS_PER_SEGMENT: #test if this works
            return -1
        # print("split index:",splitIdx)
        return splitIdx
...
def FitLine(theta, rho):
    ...
    temp=rho*np.cos(theta)
    temp1=rho*np.sin(theta)
    ...
    for i in range(n):
        for j in range(n):
            num1+=temp[i]*temp1[j]
            #using sum trig formula cos(t+b)
            den1+=temp[i]*temp[j]-temp1[i]*temp1[j]

    num1=num1*2.0/n
    den1=den1/n

    alpha=1.0/2.0*np.arctan2((num-num1),(den-den1))+np.pi/2.0
...
def MergeColinearNeigbors(theta, rho, alpha, r, pointIdx, params):
    ...
    while i < len(alpha)-1:
        ...
        alpha_merge,r_merge=FitLine(theta[startIdx:endIdx], rho[startIdx:endIdx])
        splitIdx=FindSplit(theta[startIdx:endIdx], rho[startIdx:endIdx], alpha_merge, 
        r_merge, params)
        # print("merge step: start and end",startIdx,endIdx)
        if splitIdx==-1: #merge
            # print("I accepted the merge")
            alpha[i]=alpha_merge
            alpha=np.delete(alpha,(i+1))
            r[i]=r_merge
            r=np.delete(r,(i+1))
            pointIdx[i,1]=pointIdx[i+1,1]
...
def main():
    ...
    MAX_P2P_DIST = 0.45  # max distance between two adjent pts within a segment
