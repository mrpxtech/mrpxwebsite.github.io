...
class CameraCalibrator:
    ...
    def genCornerCoordinates(self, u_meas, v_meas):
        ...
        Output:
            corner_coordinates: a tuple (Xg, Yg) where Xg/Yg is a list 
            of arrays where each array are the x/y values for each board.
        ...
        #Defining Top left corner as origin
        ...
        # #Defining Bottom left corner as origin
        # #Generating X and Y Coordinates
        # for j in reversed(xrange(self.n_corners_y)):
        #     for i in range(self.n_corners_x):
        #         a=j*self.d_square
        #         Yg.append(a)
        #         b=i*self.d_square
        ...
        x=[]
        y=[]
        #Calculate coordinates for all boards
        for z in range(self.n_chessboards):
             x.append(Xg)
             y.append(Yg)
    ...
    def estimateHomography(self, u_meas, v_meas, X, Y):    # Zhang Appendix A
        ...
        ########## Code starts here ##########
        n=np.shape(X)#n=63
        Mtilde=np.array([X,Y,np.ones(n)]) #size 3x63
        MtildeT=Mtilde.T #63x3
        zeroT=np.transpose(np.array([np.zeros(n),np.zeros(n),np.zeros(n)])) #63x3
        #reshape u and v 
        u_meas=np.reshape(u_meas, (63,1))
        v_meas=np.reshape(v_meas, (63,1))
        #Construct L matrix append method
        # L1=np.append(MtildeT,zeroT,axis=0) #126x3
        # L2=np.append(zeroT,MtildeT,axis=0) #126x3
        # L31=np.multiply(-u_meas,MtildeT) #maybe
        # L32=np.multiply(-v_meas,MtildeT)
        # L3=np.append(L31,L32,axis=0)
        # L4=np.append(L1,L2,axis=1)
        # L=np.append(L4,L3,axis=1)

        #Construct L Matrix stack method
        ...
        #Pull least singular value svd
        Q=V[8,:]
        #What is H?
        H = np.array([[Q[0],Q[1],Q[2]],[Q[3],Q[4],Q[5]],[Q[6],Q[7],Q[8]]])
        ########## Code ends here ##########
        return H
    ...
    def getCameraIntrinsics(self, H):    # Zhang 3.1, Appendix B
        ...
        #build vij
        def vbuilder(i,j,H):
            #notation is opposite. Transpose H
            H=H.T
        ...
        for i in range(self.n_chessboards):
            v12T=np.transpose(vbuilder(0,1,H[i]))
            Vl2=np.transpose(vbuilder(0,0,H[i])-vbuilder(1,1,H[i]))
            V1.append(v12T)
            V2.append(Vl2)
    ...
    def getExtrinsics(self, H, A):    # Zhang 3.1, Appendix C
        ...
        h1=np.reshape(h1, (3,1))
        h2=np.reshape(h2, (3,1))
        h3=np.reshape(h3, (3,1))
        r1=np.array(np.dot(lamda*np.linalg.inv(A),h1))
        r2=np.array(np.dot(lamda*np.linalg.inv(A),h2))
        r3=np.cross(r1,r2,axis=0)
        
        t = np.array(np.dot(lamda*np.linalg.inv(A),h3))
        R1 = np.append(r1,r2,axis=1)
        R = np.append(R1,r3,axis=1)
        
        U,S,Vh=np.linalg.svd(R, full_matrices=False)
        #not sure if this is the optimal R
        R=np.dot(U,Vh)
    ...
    def transformWorld2NormImageUndist(self, X, Y, Z, R, t):    # Zhang 2.1, Eq. (1)
        '''
        Inputs:
            X, Y, Z: the world coordinates of the points for a given board. 
                    This is an array of 63 elements X, Y come from genCornerCoordinates. 
                    Since the board is planar, we assume Z is an array of zeros.
            R, t: the camera extrinsic parameters (rotation matrix and translation vector) 
            for a given board.
        Outputs:
        ...
        '''
        ########## Code starts here ##########
        matrix=np.append(R,t,axis=1)
        n=len(X)
        Mtilde=np.row_stack((X,Y,Z,np.ones(n)))
        coor=np.dot(matrix,Mtilde)
    ...
    def transformWorld2PixImageUndist(self, X, Y, Z, R, t, A):    # Zhang 2.1, Eq. (1)
        ...
        '''
        ########## Code starts here ##########
        # Same as last problem, but add A
        matrix=np.append(R,t,axis=1)
        n=len(X)
        Mtilde=np.row_stack((X,Y,Z,np.ones(n)))
        coor=np.dot(A,matrix)
        pixcoor=np.dot(coor,Mtilde)

        #divide by third parameter to get normalized coordinates
        coornorm=np.array([pixcoor[0]/pixcoor[2],pixcoor[1]/pixcoor[2]])
    ...
    def transformWorld2NormImageDist(self, X, Y, R, t, k):    # Zhang 3.3
        ...
        ########## Code starts here ##########         
            # UPDATE ME
        x, y =transformWorld2NormImageUndist(X, Y, Z, R, t)
        k1=k[0]
        k2=k[1]

        x_br = x+x*(k1*(x**2+y**2)+k2*(x**2+y**2)**2)
        y_br = y+y*(k1*(x**2+y**2)+k2*(x**2+y**2)**2)
    ...
    def transformWorld2PixImageDist(self, X, Y, Z, R, t, A, k):    # Zhang 3.3
        ...
        u_br = u+(u+u0)*((k1*(x**2+y**2))+k2*(x**2+y**2)**2)
        v_br = v+(v+v0)*((k1*(x**2+y**2))+k2*(x**2+y**2)**2)
        ########## Code ends here ##########
        return u_br, v_br
