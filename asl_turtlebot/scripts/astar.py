...
class AStar(object):
    ...
    def solve(self):
        while len(self.open_set)>0:
            #get the vortex with the lowest f score
            ...
            #current vertex is now closed
            for neighbour in self.get_neighbors(x_current):
                if neighbour in self.closed_set: 
				    continue
                tentativeG = self.g_score[x_current] + self.distance(x_current, neighbour)
                
                if neighbour not in self.open_set:
                    self.open_set.append(neighbour) #Discovered a new vertex
                elif tentativeG >= self.g_score[neighbour]:
