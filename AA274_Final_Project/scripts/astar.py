...
class AStar(object):
        ...
        for i in range(-1,2):
            for j in range(-1,2):
                if (i==0 and j==0):
                    ...
                    node_neighbors.append((i*self.resolution+x[0],j*self.resolution+x[1]))

        
        for k in range(len(node_neighbors)):
            node_neighbors[k] = self.snap_to_grid(node_neighbors[k])
        

        # filter out nodes that are not free
        #  is_free(self, x) returns boolean true/ false

        node_neighbors_filtered = []
        for n in range(len(node_neighbors)):
            if self.is_free(node_neighbors[n]):
                node_neighbors_filtered.append(node_neighbors[n])
        
        return node_neighbors_filtered
    ...
    def solve(self):
        while len(self.open_set)>0:
            # TODO: fill me in!
            xcur = self.find_best_f_score() # find node in open set with lowest f-score
            if xcur == self.x_goal: # if robot has reach goal element
                self.path = self.reconstruct_path()
                return True # build path from the initial location to the goal location
            
            # move current node from open set to closed set
            self.open_set.remove(xcur)
            self.closed_set.append(xcur)

            for node in self.get_neighbors(xcur):
               
                if node in self.closed_set: # check if node has already been checked
                    continue # skip this node if already checked

                tentative_g_score = self.g_score[xcur] + self.distance(xcur,node)
            
                if node not in self.open_set:
                    self.open_set.append(node)
                elif tentative_g_score > self.g_score[node]:
                    continue
                
                self.came_from[node] = xcur
                self.g_score[node] = tentative_g_score
                self.f_score[node] = tentative_g_score + self.distance(node,self.x_goal)
