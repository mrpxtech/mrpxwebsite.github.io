...
FOOD_CLASSES = set(['bottle', 'wine_glass', 'cup', 'fork', 'knife', 'spoon', 'bowl', 'banana', 'apple', 'sandwich', 'orange', 'broccoli', 'carrot', 'hot_dog', 'pizza', 'donut', 'cake', 'potted_plant', 'dining_table', 'fruit', 'napkin', 'salad', 'vegetable'])
DEBUG = True
...
class Detector:
    ...
    def estimate_distance_bbox_size(self, thetaup, thetadown, thetaleft, thetaright, class_):
        if self.object_labels[class_] in NOT_FOOD_CLASSES:
            return -1, 10000000
        # ONLY APPLIES TO CLASSES PRINTED ON PAPER
        bounding_box_len = 0.17 # Measured with ruler lol
        ...
        uncertainty = 0.5 * longitudinal_dist
        return longitudinal_dist, uncertainty

    def estimate_distance(self, thetaup, thetadown, thetaleft, thetaright, ranges, class_):
        """ estimates the distance of an object in between two angles
        using lidar measurements """
        dprint("*********BEGIN DIST EST**********")
        crude_est, crude_var = self.estimate_distance_bbox_size(thetaup, thetadown, thetaleft, thetaright, class_)
        dprint("Bounding box up down left right class", thetaup, thetadown, thetaleft, thetaright, self.object_labels[class_])
        dprint("Ranges", ranges)
        dprint("Bounding box estimate & variance", crude_est, crude_var)
        #HACK
        thetaleft += math.pi + 0.0405
        thetaright += math.pi + 0.0405
        ...
        dprint("measurements before: ", meas)
        if crude_est >= 0:
            newmeas = []
            for m in meas:
                if m >= crude_est - crude_var and m <= crude_est + crude_var:
                    newmeas.append(m)
            meas = newmeas
        else:
            # look in center of window
            measlen = len(meas)
            if measlen > 8:
                meas = meas[int(measlen * 1./4):int(measlen * 3./4)]
        dprint("measurements after: ", meas)
        ...
        if num_m>0:
            ...
            dprint("bad")
            return -1
        dprint("lidar distance estimate: ", dist)
        dist = (crude_est + dist)/2.0
        dprint("average distance estimate: ", dist)
    ...
    def camera_common(self, img_laser_ranges, img, img_bgr8):
        ...
        if num > 0:
            ...
            for (box,sc,cl) in zip(boxes, scores, classes):
                ...
                dist = self.estimate_distance(thetaup, thetadown, thetaleft,thetaright,img_laser_ranges, cl)
