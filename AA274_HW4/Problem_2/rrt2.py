        ...
        for k in range(n,max_iters): 
            ...
            if self.is_free_motion(self.obstacles, x_near, x_new):
                ...
                if np.array_equal(x_new,self.x_goal):
                    sucess=True
                    solution=self.reconstructPath(V,P)
                    break
                if k == max_iters-1:
                    print('No path found at iteration:',max_iters)
                    success=False
                    break
...
class GeometricRRT(RRT):
    ...
    def find_nearest(self, V, x):
        ...
        for i in range(states):
            distance=np.linalg.norm(np.asarray(x)-np.asarray(V[i,:])) #check types
        return np.argmin(np.asarray(distance))
...
class DubinsRRT(RRT):
    ...
    def find_nearest(self, V, x):
        ...
        distance=[]
        for i in range(states):
            distance.append(path_lenght(V[i,:],x,self.turning_radius)) #check types
    ...
    def steer_towards(self, x, y, eps):
        ...
        if path(x,y,self.turning_radius) < eps:
            return y
        else:
